...
(() => {
  ...
  function sectionHTML(g) {
    ...
    const generatedNotice = n.provenance === 'model-generated · unverified'
      ? '<p class="generation-step-notice">Model generated · unverified</p>' : '';
    return `<section class="story-section" id="${g.id}" data-story-section="${g.id}" aria-labelledby="heading-${g.id}"><header class="section-heading"><span class="section-number">${String(g.number).padStart(2, '0')}</span><h2 id="heading-${g.id}">${esc(g.title)}<a class="section-anchor" href="#${g.id}" aria-label="Link to ${esc(g.title)}">#</a></h2></header>${generatedNotice}${passages.map(passageHTML).join('')}${n.transition ? `<div class="section-end">${inline(n.transition)}</div>` : ''}${extra.length ? `<details class="section-extra" data-extra="${g.id}"><summary>Supporting changes <span>· ${extra.length}</span></summary><div class="extra-body"></div></details>` : ''}<details class="review-details" data-audit="${g.id}"><summary>Invariants, tests & notes <span data-reviewed-indicator="${g.id}" class="review-check-indicator"></span></summary><div class="audit-content"></div></details></section>`;
  }
